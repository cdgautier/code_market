# reducir ifs, recorer en reverso, print(f"{1_05}"), comentarios en ingles, reemplazar create folders, numero de pag y cantidad de palabras, comparar versiones de archivos, webscraping DF.
...
class Scraper_PanelExpertos(Scraper):
    ...
    def __create_folder(self, path):
        if not os.path.exists(path):    # Se crea la carpeta si no existe
            os.makedirs(path)
    ...
    def __driver_init(self):
        self.__create_folder(self.path_discrepancys)
        prefs = {
            'download.default_directory': p.PATH_INIT_DOWNLOAD,
    ...
    def __extract_discrepancys(self):   # Método que hace webscraping de las discrepancias en la página
        ...
        for x in range(last_table):

            if (x > 0) and (x < last_table):    
            ...
            for i in range(num_rows_discrs):    # Crear iterador para carpeta de discrepancia
                ...
                if self.discrepancy_index < 9:
                    iterator = '0000' + str(i + 1)
                elif (self.discrepancy_index >= 9) and (self.discrepancy_index < 99):
                    iterator = '000' + str(i + 1)
                elif (self.discrepancy_index >= 99) and (self.discrepancy_index < 999):
                    iterator = '00' + str(i + 1)
                elif (self.discrepancy_index >= 999) and (self.discrepancy_index < 9999):
                    iterator = '0' + str(i + 1)
                else:
                    iterator = str(i)
                ...
                self.__create_folder(path_discr)    # Crea carpeta de la discrepancia
    ...
    def __extract_documents(self, path_discr):
        ...
        for i in range(num_rows_docs):  # crea identificador númerico para los documentos
            ...
            try:
                ...
                data_principal = self.__download(p.XPATH_NAME_PPAL, path_discrepancys, path_docu, p.XPATH_BTN_PPAL_DWLD) 
    ...
    def __extract_adjoints(self, path_attach, id_general):
        ...
        try:
            ...
            self.__create_folder(path_attach)   # Crear carpeta de adjuntos
            # click en adjunto
            WebDriverWait(driver, p.WAIT_TIME_6).\
            ...
            for i in range(num_rows_adjo):
                ...
                # descarga los archivos en las rutas de destino y devuelve la data de el archivo
                data_adjoint = self.__download(xpath_name_adjo, path_discrepancys, path_attach, xpath_btn_adjo_dwld)
